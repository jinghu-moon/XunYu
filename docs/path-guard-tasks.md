...
- [ ] **T0-2** 在 `src/lib.rs` 中注册 `mod path_guard;`
- [ ] **T0-3** 创建目录 `src/path_guard/` 及以下占位文件：
...
- [ ] **T0-4** 确认 `Cargo.toml` 依赖（无需新增 feature 开关）：
...
- [ ] **T1-2** 定义完整 `PathIssueKind` 枚举（含 Win32 错误码注释，不省略）：
  ```rust
...
- [ ] **T1-6** 为 `PathPolicy` 实现 3 个预设 builder：
...
- [ ] **T1-7** 为 `PathPolicy` 实现 `Default`，等同于 `for_read()`

- [ ] **T1-8** 单元测试：builder 各字段值正确；`Default` 与 `for_read()` 等价
...
- [ ] **T2-1** 实现 `pub(crate) fn reserved_names() -> &'static [&'static str]`：
...
- [ ] **T2-2** 实现 `pub(crate) fn detect_kind(path: &str) -> PathKind`（FSM，字节级匹配）：
...
- [ ] **T2-3** 实现 `pub(crate) fn is_ads(path: &str, kind: PathKind) -> bool`：
  - 查找 index != 1 处的 `:`；UNC/ExtendedLength 路径跳过盘符位检测

- [ ] **T2-4** 实现 `pub(crate) fn check_chars(path: &str) -> Option<PathIssueKind>`：
  - 空字符串 → `Empty`
  - 控制字符 0x00~0x1F（字节扫描，编译器自动向量化）→ `InvalidChar`
  - 非法字符 `< > " | ? *`（字节扫描）→ `InvalidChar`

- [ ] **T2-5** 实现 `pub(crate) fn check_component(component: &str) -> Option<PathIssueKind>`：
  - 末尾为空格或 `.` → `TrailingDotSpace`
  - stem（第一个 `.` 前的部分）lowercase 后在 `reserved_names()` 中 → `ReservedName`

- [ ] **T2-6** 实现 `pub(crate) fn check_traversal(base: &Path, joined: &Path) -> Option<PathIssueKind>`：
  - `joined.strip_prefix(base).is_err()` → `TraversalDetected`（纯字符串语义，不访问磁盘）

- [ ] **T2-7** 实现主入口 `pub(crate) fn check_string(raw: &str, policy: &PathPolicy) -> Option<PathIssueKind>`，严格按以下顺序：
  1. `check_chars`
...
- [ ] **T3-1** 实现 `fn to_wide_with_prefix(path: &Path) -> Vec<u16>`：
...
- [ ] **T3-3** 实现内部函数 `fn probe_path(wide: &[u16]) -> Result<u32, PathIssueKind>`：
...
- [ ] **T3-4** 实现 `pub(crate) fn probe(path: &Path) -> Result<u32, PathIssueKind>`：
  - 使用 TLS 缓冲区，调用 `probe_path`

- [ ] **T3-5** 实现 `pub(crate) fn is_reparse_point(attr: u32) -> bool`
- [ ] **T3-6** 实现 `pub(crate) fn is_directory(attr: u32) -> bool`

- [ ] **T3-7**（增强阶段）`get_full_path`：调用 `GetFullPathNameW`，MVP 用 `cwd.join(rel)` 替代

- [ ] **T3-8** 单元测试：`to_wide_with_prefix` 前缀正确、不重复；`probe` 存在/不存在路径；属性位辅助函数
...
- [ ] **T4-5** `mod.rs` re-export 所有公共类型

- [ ] **T4-6** 单元测试（`must_exist=false`，无需真实文件系统）：
  - 去重顺序稳定；合法/非法混合分类正确；`allow_relative=false` 时相对路径进 issues

- [ ] **T4-7** 集成测试（`tests/` 目录，需真实文件系统）：
...
- [ ] **T5-1** 在 `src/lib.rs` 中将 `mod path_guard` 注册为无条件模块（与 `mod windows` 同级）
- [ ] **T5-2** 修改 `src/commands/delete/filters.rs`：
...
- [ ] **T6-1** 修改 `src/commands/acl_cmd/edit.rs` 的 `cmd_add`：
...
- [ ] **T6-2** 验证 `acl add -p <valid_path>` 正常工作
- [ ] **T6-3** 验证 `acl add -p <reserved_name>` 返回明确错误
- [ ] **T6-4** 验证 `acl add --paths a,b,a` 去重后只处理 a、b 两条
- [ ] **T6-5** 验证 `acl add -p "C:\Windows\System32"` 因 `safety_check` 被拒绝

---

## 阶段 7：性能优化（MVP 阶段可顺手完成）

- [ ] **T7-1** `winapi.rs`：确认 TLS 缓冲区 `RefCell<Vec<u16>>` 与 rayon worker 线程兼容（无 `Send` 问题）
- [ ] **T7-2** `string_check.rs`：`check_chars` 使用字节级扫描（`path.as_bytes().iter()`），确认编译器自动向量化（release 模式检查汇编或 benchmark）
- [ ] **T7-3** `string_check.rs`：FSM `detect_kind` 使用字节 slice 模式匹配，不用 `starts_with(&str)`
- [ ] **T7-4** `mod.rs`：去重时只对 normalize key 分配，原始路径直接 move 进 `ok`，避免克隆
- [ ] **T7-5** `mod.rs`：`PathIssue.detail` 字段使用 `&'static str`，不分配堆内存

---

## 阶段 8：并行管线（增强阶段）

- [ ] **T8-1** `parallel.rs`：实现并行阈值分支逻辑：
  - `< 64` 条 → 直接调用串行 `validate_paths`
  - `64~500` 条 → `rayon::par_iter()` 字符串校验 + WinAPI 探测
  - `> 500` 且含 UNC → UNC 路径分流，降并发至 `min(4, num_cpus::get())`
- [ ] **T8-2** `parallel.rs`：WinAPI 阶段使用 `crossbeam-channel`（项目已有依赖）构建固定线程数 worker 池，避免 IO 阻塞 Rayon
- [ ] **T8-3** `parallel.rs`：`cwd_snapshot` 在进入并行前快照一次，所有 worker 共享同一个 `Arc<PathBuf>`
- [ ] **T8-4** 更新 `mod.rs` 入口，按阈值选择串行或并行路径
- [ ] **T8-5** 性能基准（`benches/` 或 `tests/` 下手动 benchmark）：
  - 记录运行环境（CPU、磁盘类型）
...
- [ ] **T9-1** `winapi.rs`：实现 `get_full_path(path: &Path) -> Result<PathBuf, PathIssueKind>`
  - 调用 `GetFullPathNameW`，替换 T4-2 的 `cwd.join()` 方案
- [ ] **T9-2** `winapi.rs`：实现 `open_path_with_policy`（`CreateFileW`）：
...
- [ ] **T9-3** `winapi.rs`：实现 `ExpandEnvironmentStringsW` 封装（T4-3 的完整版）
- [ ] **T9-4** P0 命令接入 `open_path_with_policy`（`delete`、`acl remove/repair`）
...
- [ ] **T10-1** P0：`delete` 命令接入 `PathPolicy::for_write()`
- [ ] **T10-2** P0：`acl remove/owner/repair/copy/restore` 接入
- [ ] **T10-3** P0：`redirect --apply/--undo` 接入
- [ ] **T10-4** P0：`vault/crypt` 输出路径接入 `PathPolicy::for_output()`
- [ ] **T10-5** P1：`bak restore` 接入
- [ ] **T10-6** P1：`img` 输出目录、`brn --apply` 接入
- [ ] **T10-7** P2：`find`/`cstat`/`diff` 接入 `PathPolicy::for_read()`
- [ ] **T10-8** P3：`bookmarks set`/`ctx set` 接入（`must_exist=false`）

---

## 阶段 11：测试与 CI

- [ ] **T11-1** 单元测试覆盖率：所有 `PathIssueKind` 变体至少有一个触发用例
- [ ] **T11-2** 集成测试（`tests/path_guard_integration.rs`）：
  - `--file` 输入（多行路径文件）
  - `--paths` 逗号列表
  - 权限受限目录的 `AccessDenied` 分类
  - UNC 路径（若测试环境有网络共享）
- [ ] **T11-3** Fuzz 测试（`cargo-fuzz`，可选）：针对 `check_string` 的随机输入边界
- [ ] **T11-4** CI：Windows runner（`windows-latest`）执行全部单元 + 集成测试
- [ ] **T11-5** CI：release 模式编译验证（`opt-level=z` + `strip=symbols`）
