...
fn acl_view_detail_and_export_csv() {
    ...
    run_ok(acl_cmd(&env).args([
        "acl", "view", "-p", &str_path(&dir),
        "--export", &str_path(&export),
    ]));
...
fn acl_diff_reports_inheritance_diff() {
    ...
    run_ok(acl_cmd(&env).args([
        "acl", "inherit", "-p", &str_path(&dir_a),
        "--disable", "--preserve", "false",
    ]));
    ...
    let err = stderr_str(&out);
    assert!(err.contains("Inheritance differs"), "missing inheritance diff: {err}");
}
...
fn acl_diff_same_path_reports_zero_diff() {
    ...
    let err = stderr_str(&out);
    assert!(err.contains("Only in A: 0"), "expected zero only-in-A: {err}");
    assert!(err.contains("Only in B: 0"), "expected zero only-in-B: {err}");
}
...
fn acl_effective_outputs_masks_for_user() {
    ...
    let out = run_ok(acl_cmd(&env).args([
        "acl", "effective", "-p", &str_path(&dir),
        "-u", "BUILTIN\\Users",
    ]));
    let err = stderr_str(&out);
    assert!(err.contains("User: BUILTIN\\Users"), "missing user header: {err}");
    assert!(err.contains("specified user only"), "missing user-only note: {err}");
}
...
fn acl_effective_deny_overrides_allow_cli() {
    ...
    let err = stderr_str(&out);
    assert!(
        err.lines().any(|line| line.contains("Delete") && line.contains("Deny")),
