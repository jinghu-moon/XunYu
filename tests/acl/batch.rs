...
fn acl_add_batch_file_writes_audit() {
    ...
    let content: String = dirs.iter().map(|p| format!("{}
", str_path(p))).collect();
    std::fs::write(&list_path, content).unwrap();

    run_ok(acl_cmd(&env).args([
        "acl", "add", "--file", &str_path(&list_path),
    ...
    assert_eq!(add_paths.len(), dirs.len(), "expected one audit entry per path");
    for path in &dirs {
        let path_str = str_path(path);
        assert!(add_paths.iter().any(|p| p == &path_str), "missing audit entry for {path_str}");
    }
}
...
fn acl_add_batch_with_missing_path_reports_error() {
    ...
    let mut content: String = dirs.iter().map(|p| format!("{}
", str_path(p))).collect();
...
", str_path(&missing)));
    ...
    let add_paths = read_audit_paths_for_action(&env, "AddPermission");
    assert!(add_paths.is_empty(), "expected no audit entries when validation fails");
}
...
", str_path(p))).collect();
    ...
    let add_paths = read_audit_paths_for_action(&env, "AddPermission");
    assert_eq!(add_paths.len(), dirs.len(), "expected audit entries for all paths");
    for path in &dirs {
        let path_str = str_path(path);
        assert!(add_paths.iter().any(|p| p == &path_str), "missing audit entry for {path_str}");
    }
