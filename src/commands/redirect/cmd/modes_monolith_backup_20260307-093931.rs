...
use super::super::plan::{PlanItem, PlanKind};
use super::super::prompt::{confirm_overwrite_for_profile, prompt_conflicts};
use super::super::render::{
    render_dry_run_summary, render_preview_summary, render_results, render_simulate_results,
    render_stats, render_undo_results,
};
use super::super::{debug_tools, engine, redirect_log, undo, watcher};
use super::format::resolve_format;

pub(super) fn run_log(args: &RedirectCmd) -> CliResult {
    let format = resolve_format(&args.format)?;
    let items = redirect_log::query_tx_summaries(args.tx.as_deref(), args.last);
    redirect_log::render_tx_summaries(&items, format);
    Ok(())
}

pub(super) fn run_explain(profile: &crate::config::RedirectProfile, name: &str) -> CliResult {
...
pub(super) fn run_undo(
    ...
    let tx = if tx.eq_ignore_ascii_case("select") || tx == "-" {
        ...
        let items = redirect_log::query_tx_summaries(None, Some(20));
        if items.is_empty() {
            return Err(err2(
...
pub(super) fn ensure_overwrite_confirm(
    args: &RedirectCmd,
    profile: &crate::config::RedirectProfile,
) -> CliResult {
...
pub(super) fn ensure_source_dir(source: &Path, apply_mode: bool) -> CliResult {
    if apply_mode {
        return Ok(());
    }
    if !source.exists() {
...
pub(super) fn resolve_source(source: Option<&str>) -> PathBuf {
    source
        .map(PathBuf::from)
