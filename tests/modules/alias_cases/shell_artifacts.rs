...
fn alias_import_duplicate_app_targets_rebuilds_both_shell_entries() {
    ...
    assert!(
        toml.contains("[app.code]"),
        "code app alias missing after import"
    );
    assert!(
        toml.contains("[app.editor]"),
        "editor app alias missing after import"
    );
    assert_shim_exists(&dst_env, "code");
...
fn alias_import_without_changes_does_not_rewrite_shell_artifacts() {
    ...
    assert_eq!(
        cmd_mtime_1, cmd_mtime_2,
        "cmd macrofile should not be rewritten on noop import"
    );
    assert_eq!(
        ps_mtime_1, ps_mtime_2,
        "powershell profile should not be rewritten on noop import"
    );
}
