...
**推荐方案：backup 直接支持多格式输出，A 作为增强项后置**
...
| **A. 7-Zip 格式插件 DLL（只读）** | 用户用 7-Zip 直接浏览/解压 `.xunbak` | Rust PoC / C++ 薄壳 + Rust core 正式版 | Phase 4（增强项） |
...
> **来源**：7-Zip 26.00 `CPP/7zip/Archive/IArchive.h`。方法 7-10 在 PoC 阶段可返回 `S_OK` + 空值。
...
| **分卷支持** | 7-Zip 的多卷入口是 `IArchiveOpenVolumeCallback`，由 7-Zip 按需回调获取下一卷流。当前 reader 的 `discover_split_volumes` 依赖文件系统枚举，需重构为回调驱动模式。PoC 不支持分卷。 |
...
└── xunbak/
    └── export/
        ...
        ├── dir_writer.rs          # --format dir（复用 restore 语义；首版不自研）
        ├── zip_writer.rs          # --format zip（首版只支持 stored|deflated）
        ├── sevenz/
        │   ├── mod.rs             # SevenZExportWriter 入口
        │   ├── header.rs          # 首版：单卷/分卷共用最小 header 写出
        │   ├── methods.rs         # 首版：copy|lzma2；扩展留到后续
        │   ├── pack_streams.rs    # pack info / unpack info / substreams
        │   ├── writer.rs          # SevenZWriter：逻辑 7z 输出（首版 non-solid）
...
> **边界约束**：B1（zip）直接使用成熟 `zip` crate，不自研 ZIP 格式层；源码参考只用于理解能力边界，不作为手写 ZIP writer 的依据。
...
**技术路径**：
1. **B1: zip 导出**：用 Rust `zip` crate 生成标准 `.zip`（最简单，无外部依赖）
2. **B2: 7z 导出（正式路线）**：参考 `refer/sevenz-rust2-main`，在仓库内自实现标准 `.7z` 写出模块。纯 Rust 实现，支持 LZMA/LZMA2/BZIP2/PPMd，无需 C 编译器
...
> **为什么 B2 参考 `sevenz-rust2` 而不直接依赖它**：目标是保持主仓库对 `.7z` 导出的能力完全可控，减少外部 crate 版本漂移与 API 变更风险。`refer/sevenz-rust2-main` 已证明“纯 Rust 写 `.7z`”可行，可作为设计与测试参考；实现仍落在 xun 自己的模块中。

### 方案 B1 深入理解（基于 `refer/zip2-master`）

**结论**：B1 直接使用成熟 `zip` crate，不自研 ZIP 格式层。
...
| 压缩方法 | `compression_method()` | 支持 `Stored / Deflated / Bzip2 / Xz / Zstd / PPMd` 等 |
...
1. B1 **不自研 ZIP writer**
2. B1 默认使用 `ZipWriter::new(File)`，不优先走 `new_stream()`
3. B1 默认压缩方法建议使用 **Deflated**
4. 若用户明确要求最大兼容性，可增加 `--method stored`
5. B1 不承诺 Windows 属性完全保真；只保证路径、内容、目录结构、mtime 和常规权限语义
6. **B1 不支持 `--split-size`**，因为 `zip` crate 当前不支持 multi-disk ZIP
7. 虽然 `zip` crate 支持更多压缩方法，但 **XunYu 首版 ZIP 导出只暴露 `stored | deflated`**
...
1. 缩小构建体积和编译时间
2. 降低实现和兼容性复杂度
3. 把 B1 明确收敛为“标准 ZIP 互操作”，而不是“全功能 ZIP 工具”
...
5. ZIP 模式下如需统一兼容性，默认只开放 `stored | deflated`

> 其余 `zip` crate 已支持的方法（如 `bzip2/xz/zstd/ppmd`）不进入首版 CLI 契约，避免兼容性和依赖集合失控。
...
| 自定义压缩方法链 | `set_content_methods()` | 可组合 LZMA/LZMA2/BZIP2/PPMd，以及可选 Brotli/Deflate/LZ4/Zstd |
...
5. 实现策略建议：优先自研最小写出子集（目录/文件、LZMA2、non-solid、时间戳/属性），加密和可选 codec 后置
...
2. `seek()` **不承诺完全通用**；首版只保证满足 7z writer 的必要 seek 场景：
   - 初始化预留 header 后继续顺序写
...
> 若后续需要支持任意随机 seek，需单独引入卷索引、按卷 reopen 和全局偏移到卷内偏移映射；这不属于首版范围。

**句柄管理策略**：

1. 首版仅长期持有**当前卷**的 `File`
...
| `--level` | int | 压缩等级 |
| `--threads` | int | 压缩线程数 |
| `--password` | string | 仅 `7z` 首版可选；`zip` 首版禁用 |
...
4. `zip` 模式下默认 `--method deflated`；若用户显式指定 `stored` 则关闭压缩
5. `zip` 模式下不支持 `--split-size`，因为 `zip` crate 当前不支持 multi-disk ZIP
6. `zip` 模式下不启用 `--password` / `--encrypt-header` 作为首版范围
...
| 输入 | 处理 |
|------|------|
| `backup create --format zip --method lzma2` | 立即报参数错误 |
...
| 依赖 | `zip` crate（B1）、仓库内自研 `.7z` 写出模块（B2，参考 `refer/sevenz-rust2-main`）、或用户系统中的 7-Zip（B3，自动探测 `7z.exe` / `7za.exe` / `7zr.exe`） |
| 分发 | 内置于 xun CLI |
| 维护 | 低 |
...
> **推荐顺序**：先做 B1（zip），再做 B2（参考 `sevenz-rust2` 自研纯 Rust `.7z` 写入）；B3 仅作为低成本 fallback，不建议作为长期唯一方案。B 全链路纯 Rust，无 C/C++ 工具链依赖。
...
| 解压单文件 | xun restore --file | 7-Zip 拖拽 | 标准工具 |
| 解压全部 | xun restore | 7-Zip 解压到 | 标准工具 |
| 增量更新 | 原生支持 | 不支持 | 不支持 |
...
Phase 2
  └── 方案 B1（优先）：xun backup --format zip
      └── 用 Rust zip crate 生成标准 zip（~500 行，无外部依赖）
...
Phase 4
  └── 方案 A PoC：crates/xunbak-7z（只读 DLL）
      ├── 范围：单文件 .xunbak，无分卷，锁定单一 7-Zip 版本（如 26.00）
      ├── 无前置重构：直接调用底层模块，不走 ContainerReader::open
      ├── 纯 Rust PoC：COM vtable 定义 + 导出函数 + handler + InStream 适配
      ├── 支持 zstd + none codec
      ├── 只实现 list + extract 核心功能
      └── GetNumberOfProperties / GetArchivePropertyInfo 返回空值
...
| 导出格式 | 依赖 | 许可义务 |
|---------|------|---------|
| zip | Rust `zip` crate（MIT） | 无 |
| 7z（仓库内自研，参考 `sevenz-rust2`） | 纯 Rust，自有实现 | 无新增外部二进制义务 |
...
**决定**：方案 B 全链路纯 Rust，无 C/C++ 工具链依赖。推荐落地顺序：先 B1（`zip` crate），再 B2（参考 `sevenz-rust2` 自研 `.7z` 写出模块）；B3（自动探测 `7z.exe` > `7za.exe` > `7zr.exe`）仅作为过渡 fallback。C/C++ 仅在方案 A 正式版中作为 COM 薄壳出现（见 §6.9）。
