...
impl BakProject {
    fn new(name: &str, compress: bool) -> Self {
        ...
        let cfg = if compress { CFG_COMPRESS } else { CFG_NO_COMPRESS };
        ...
        fs::read_dir(&backups)
            ...
            .find(|e| {
                ...
                n.starts_with(prefix)
                    && ext.map(|ex| n.ends_with(ex)).unwrap_or(true)
            })
...
fn restore_cmd_to_dir() {
    ...
    assert!(dest.join("a.txt").exists(), "a.txt should exist in --to dir");
    assert!(dest.join("b.txt").exists(), "b.txt should exist in --to dir");
...
fn restore_cmd_glob_txt_only() {
    ...
    assert!(root.join("a.txt").exists(), "a.txt should be restored by glob");
...
fn restore_cmd_snapshot_creates_pre_restore() {
    ...
    assert!(has_pre_restore, "pre_restore snapshot backup should be created");
}
...
fn restore_cmd_dry_run() {
    ...
    assert!(!proj.root.join("a.txt").exists(), "dry-run should not restore files");
}
...
fn restore_cmd_nonexistent_backup_fails() {
    ...
    let out = env.cmd().args([
        "restore", "nonexistent-backup-xyz",
        "-C", root.to_str().unwrap(),
        "-y",
    ]).output().unwrap();
    assert!(!out.status.success(), "should fail for nonexistent backup");
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(stderr.contains("not found") || stderr.contains("Backup"),
...
fn restore_cmd_file_from_zip_backup() {
    ...
    assert!(proj.root.join("a.txt").exists(), "a.txt should be restored from zip");
    assert_eq!(fs::read_to_string(proj.root.join("a.txt")).unwrap(), "aaa");
    // b.txt 应未受影响
    assert!(proj.root.join("b.txt").exists());
    assert_eq!(fs::read_to_string(proj.root.join("b.txt")).unwrap(), "bbb");
...
fn restore_cmd_file_unsafe_path_fails() {
    ...
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(stderr.contains("Unsafe") || stderr.contains("unsafe") || stderr.contains("Invalid"),
...
fn restore_cmd_glob_double_star() {
    ...
    let name = fs::read_dir(root.join("A_backups")).unwrap().flatten()
        .find(|e| e.file_name().to_string_lossy().starts_with("v1-"))
        .unwrap().file_name().to_string_lossy().into_owned();
    ...
    // .ts 文件应被还原
    assert!(src_dir.join("button.ts").exists(), "button.ts should be restored");
    assert!(src_dir.join("icon.ts").exists(), "icon.ts should be restored");
    // .rs 文件不应被还原（不匹配 glob）
    assert!(!root.join("src").join("index.rs").exists(),
...
fn restore_cmd_glob_from_zip() {
    ...
    assert!(proj.root.join("a.txt").exists(), "a.txt should be restored from zip via glob");
...
fn restore_cmd_preserves_nested_directory_structure() {
    ...
    // 嵌套结构应完整重建
    assert!(deep.join("helper.txt").exists(), "nested helper.txt should be restored");
