...
fn acl_inherit_copy_backup_restore_and_batch() {
    ...
    run_ok(acl_cmd(&env).args([
        "acl", "copy", "-p", &str_path(&dir_b), "-r", &str_path(&dir_a), "-y",
    ]));
    ...
    assert!(count_acl_backups(&export_dir) >= 2, "expected backups in export dir");

    let actions = read_audit_actions(&env);
    for action in ["SetInheritance", "CopyAcl", "BackupAcl", "RestoreAcl", "Batch"] {
        assert!(actions.iter().any(|a| a == action), "missing {action} audit entry");
    }
}
...
fn acl_inherit_enable_and_preserve_false() {
    ...
    run_ok(acl_cmd(&env).args([
        "acl", "inherit", "-p", &str_path(&dir),
        "--disable", "--preserve", "false",
    ]));

    let out = run_ok(acl_cmd(&env).args(["acl", "view", "-p", &str_path(&dir)]));
    let err = stderr_str(&out);
    assert!(err.contains("Inherit: disabled"), "expected inheritance disabled: {err}");

    run_ok(acl_cmd(&env).args(["acl", "inherit", "-p", &str_path(&dir), "--enable"]));

    let out = run_ok(acl_cmd(&env).args(["acl", "view", "-p", &str_path(&dir)]));
    let err = stderr_str(&out);
    assert!(err.contains("Inherit: enabled"), "expected inheritance enabled: {err}");
}
