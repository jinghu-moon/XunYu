...
use super::modules::{
    enumerate_process_modules, enumerate_process_modules_with_initial_slots, match_pids_by_modules,
    modules_match_targets,
};
...
impl DebugPrivilegeApi for FakeDebugPrivilegeApi {
    unsafe fn open_process_token(&self, _process: HANDLE, _access: u32, token: *mut HANDLE) -> i32 {
        if self.open_ok {
            if !token.is_null() {
                unsafe {
                    *token = 1 as HANDLE;
    ...
    unsafe fn adjust_token_privileges(&self, _token: HANDLE, _new_state: *const TOKEN_PRIVILEGES) {}
