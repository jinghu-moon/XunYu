...
fn minimal_png() -> Vec<u8> {
    vec![
        ...
        0xFF, 0xFF, 0xFF, 0x7F, 0x00, 0x09, 0xFB, 0x03, 0xFD, 0x05, 0x43, 0x45, 0xCA, 0x00, 0x00,
...
fn minimal_jpeg() -> Vec<u8> {
    vec![
        ...
        0x14, 0x14, 0x14, 0x14, 0x14, 0x14, 0x14, 0x14, 0x14, 0x14, 0x14, 0x14, 0x14, 0x14, 0x14,
        0x14, 0x14, 0x14, 0x14, 0x14, 0x14, 0x14, 0x14, 0xFF, 0xC0, 0x00, 0x11, 0x08, 0x00, 0x01,
        ...
        0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x03, 0x04,
        ...
        0xEA, 0xF2, 0xF3, 0xF4, 0xF5, 0xF6, 0xF7, 0xF8, 0xF9, 0xFA, 0xFF, 0xDA, 0x00, 0x0C, 0x03,
        0x01, 0x00, 0x02, 0x11, 0x03, 0x11, 0x00, 0x3F, 0x00, 0xF9, 0xD2, 0x8A, 0x28, 0xAF, 0xC3,
...
fn minimal_wav() -> Vec<u8> {
    vec![
        0x52, 0x49, 0x46, 0x46, 0x28, 0x00, 0x00, 0x00, 0x57, 0x41, 0x56, 0x45, 0x66, 0x6D, 0x74,
...
fn minimal_ogg() -> Vec<u8> {
    vec![
        0x4F, 0x67, 0x67, 0x53, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
fn minimal_mp4() -> Vec<u8> {
    vec![
        ...
        0x00, 0x69, 0x73, 0x6F, 0x6D, 0x69, 0x73, 0x6F, 0x32, 0x00, 0x00, 0x00, 0x08, 0x6D, 0x64,
        0x61, 0x74,
    ]
}
...
fn assert_filevault_roundtrip(
    ...
    assert_eq!(
        verify_json["status"], "ok",
        "verify should succeed for {file_name}"
    );
    ...
    assert_eq!(
        fs::read(&out).unwrap(),
        bytes,
        "roundtrip bytes must match for {file_name}"
    );
}
...
fn filevault_roundtrips_common_file_formats() {
    ...
    let cases: Vec<(&str, Vec<u8>)> = vec![
        (
            "readme.txt",
            "FileVault common format coverage\r\n第二行"
                .as_bytes()
                .to_vec(),
        ),
        (
        ...
        (
            "config.json",
            br#"{"name":"filevault","ok":true,"count":13}"#.to_vec(),
        ),
        (
            "lines.jsonl",
            b"{\"event\":\"open\"}\n{\"event\":\"close\"}\n".to_vec(),
        ...
        (
            "page.html",
            b"<!doctype html><html><body><h1>FileVault</h1></body></html>".to_vec(),
        ),
        (
            "diagram.svg",
        ...
        (
            "doc.xml",
            b"<?xml version=\"1.0\"?><root><item>FileVault</item></root>".to_vec(),
        ),
        ...
        (
            "archive.zip",
            zip_bytes(&[
                ("hello.txt", b"zip-entry-1"),
                ("nested/data.bin", &[1, 2, 3, 4, 5]),
            ]),
        ),
