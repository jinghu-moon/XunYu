...
`P3` 到 `P5` 的目标不是把更多 CLI 子命令平铺成新页面，而是把既有能力稳定收口到 8 个工作台体系内，并补齐跨工作台协作链路。

本轮完成的重点有三类：

- 为 `路径与上下文`、`集成与自动化`、`媒体与转换` 补齐任务与 Recipe 收口
- 为多个工作台补齐“本地 Recent Tasks -> 全局统计诊断”的跳转桥
...
- 顶层导航不再继续膨胀成命令墙
- 新能力优先以“工作流卡片 + Recipe + 最近任务”进入既有工作台
- 任务定义、目标路径、dry-run / guarded 协议继续复用同一套模型
...
- `TaskToolbox` 承载 `ctx / workspace / recent / stats / dedup / check / gc / keys / all / fuzzy` 等任务型能力
...
- `audit` / `diagnostics-center` 事件统一上抛给 `App.vue`
...
本轮最关键的收口是把“任务卡 / Recipe / 最近任务 -> 统计诊断”的跨工作台桥补齐。
...
- 若目标是跨工作台诊断消费，`App.vue` 自动切到 `statistics-diagnostics`
...
这保证了对应工作台在 UI 上不再只有任务卡，也具备可直接复用的顺序工作流模板。
...
- `App.vue` 能把任意工作台的 `link-panel` 正确转发到统计诊断工作台
...
- `audit / diagnostics-center` 事件会上抛到全局桥
...
- `Phase 0`：8 工作台骨架、统一确认弹窗、任务回执、最近任务、Recipe、诊断中心已完成
- `Phase 1`：文件与安全工作台已具备文件管理、上下文桥接、任务区、最近任务与 Recipe 收口
- `Phase 2`：文件治理观察面、治理摘要、批量治理和统计诊断联动已完成第一阶段闭环
- `Phase 3`：路径与上下文工作台已完成任务化收口
- `Phase 4`：集成与自动化工作台已完成任务化收口
- `Phase 5`：媒体与转换工作台完成落地，统计诊断工作台完成跨域消费层收口
...
- 本地闭环优先，跨域诊断统一走 `Statistics & Diagnostics`
