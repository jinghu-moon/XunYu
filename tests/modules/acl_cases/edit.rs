...
fn acl_add_and_purge_write_audit() {
    ...
    assert!(
        has_acl_row(
            &rows, "Allow", "显式", principal, "Read", "None", "None", "否"
        ),
...
fn acl_add_overwrites_existing_allow_for_same_principal() {
    ...
    assert!(
        has_acl_row(
            &rows, "Allow", "显式", principal, "Modify", "None", "None", "是"
        ),
        "missing overwritten Modify ACE in export rows"
    );
    assert!(
        !has_acl_row(
            &rows, "Allow", "显式", principal, "Write", "None", "None", "是"
        ),
...
fn acl_remove_non_interactive_by_principal() {
    ...
    assert!(
        has_acl_row(
            &rows, "Allow", "显式", principal, "Read", "None", "None", "是"
        ),
