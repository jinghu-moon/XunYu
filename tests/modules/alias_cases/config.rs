...
fn alias_add_force_replaces_existing_app_alias() {
    ...
    let toml = read_toml(&env);
    assert!(
        toml.contains("[alias.switcher]"),
        "shell alias missing after force replace"
    );
    assert!(
        !toml.contains("[app.switcher]"),
        "app alias should be removed after force replace"
    );
...
fn alias_import_force_replaces_existing_app_alias() {
    ...
    run_ok(alias_cmd(&env).args(["alias", "import", import_path.to_str().unwrap(), "--force"]));
